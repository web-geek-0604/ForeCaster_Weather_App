...
function App() {
  ...
  const handleOnSearchChange = (searchData) => {
    ...
    const CurrentPollution = fetch(`http://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${WEATHER_API_KEY}`);
  ...
  return (
    <div className="container">
      <div className='mx-auto my-[50px] p-4 flex flex-wrap items-center justify-center text-3xl sm:text-5xl'>
        <div className='flex items-center justify-center'><h1 className='text-center text-white  font-ubuntu'>F</h1><img className='w-[50px] mt-[15px]' src="icons/snowflake.svg" alt="snowflake" /></div><div><h1 className='text-center text-white font-ubuntu'>reCaster : Weather App</h1></div>
      </div>
